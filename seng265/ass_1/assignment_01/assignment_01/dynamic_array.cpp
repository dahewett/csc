...
void Dynamic_array::insert(Dynamic_array &p, int i) {						//-
	...
	// case 3: array empty									//-
	if (size == 0) {
		Block *init_copy = copy_blocks(p.head_p);
		insert_blocks(head_p, init_copy );
		size += p.size;
		return;
	}
	...
	if (position.block_p->size + p.size < BLOCK_SIZE) {
		for (int j = position.i; j < position.i + p.size; j++) {
			position.block_p->a[j+p.size] = position.block_p->a[j];
		}
		
		for (int k = 0; k < p.get_size(); k++) {
			position.block_p-> a[position.i + k] = p[k];
			size++;
			position.block_p->size++;
		}
	}
	...
	// case 5: array non-empty; new blocks needed						//-
		...
		if (position.i > 0 && position.i < BLOCK_SIZE - 1) {
			Block * new_block_p = new Block;
			...
			position.block_p-> size = (position.i+1);
			insert_blocks(position.block_p, new_block_p);
...
void Dynamic_array::remove(int i) {								//-
	...
	// case 3: block size > 1								//-
	if( position.block_p->size > 1) {
		for(int j = position.i; j < position.block_p->size-1; j++){
			position.block_p->a[j] = position.block_p->a[j+1];
		}
...
void Dynamic_array::remove(int start, int end) {						//-
	...
	// find target blocks and indexs and mod5 of start and end
	int n_start = start%5;
	int n_end = end%5;
	Block_position s_position = find_block(start);
	Block_position e_position = find_block(end);
	
	//if delete entire
	if (start == 0 && end == size) {
		remove_blocks(s_position.pre_block_p, s_position.block_p, e_position.block_p);
		size -= (end-start);
		return;
	}
	
	//case 3: in the same block
	if (s_position.block_p == e_position.block_p) {
		
		for(int j = n_start; j < e_position.block_p->size; j++) {
			if ( j + n_end > e_position.block_p->size) {
				s_position.block_p->a[j] = 0;
			}
			s_position.block_p->a[j] = s_position.block_p->a[j+n_end-1];
		}
		s_position.block_p->size -= (n_end-n_start);
	
		if (s_position.block_p->size == 0) {
			remove_blocks(s_position.pre_block_p, s_position.block_p, s_position.block_p);
		}
		size -= (end-start);
		return;
		
		//case 4: in multiple blocks
	} else {
		//case 4.a: if start is at begining node
		s_position.block_p->size = n_start;
		for (int j = 0; j < e_position.block_p->size; j++) {
			if ( j + n_end > e_position.block_p->size ) {
				e_position.block_p->a[j] = 0;
			}
			e_position.block_p->a[j] = e_position.block_p->a[j+n_end];
		}
		e_position.block_p->size -= n_end;
		
		//start and end block empty
		if (e_position.block_p->size == 0 && s_position.block_p->size == 0){
			remove_blocks(s_position.pre_block_p, s_position.block_p, e_position.block_p);
		}
		else if ( s_position.block_p->next_p == e_position.block_p) {
			if (e_position.block_p->size == 0) {
				remove_blocks(e_position.pre_block_p, e_position.block_p, e_position.block_p);
			}
			if ( s_position.block_p->size == 0) {
				remove_blocks(s_position.pre_block_p, s_position.block_p, s_position.block_p);
			}
		...
		//end of the block
			...
			if (s_position.block_p->size == 0) {
				remove_blocks (s_position.pre_block_p,s_position.block_p->next_p,e_position.pre_block_p);
			}
			else {
				remove_blocks (s_position.block_p, s_position.block_p->next_p, e_position.pre_block_p);
			}
		}
		size -= (end-start);
		return;
